...
# ===== STORE ORDER & DEPOT =====
#  – Force the exact order you want (PatraPada included; depot last)
store_list = [
    "Sum Hospital",
    "Aiims",
    "Bapuji Nagar",
...
def solve_vrp(data):
    ...
    dcb = model.RegisterTransitCallback(dist_cb)
    model.SetArcCostEvaluatorOfAllVehicles(dcb)

    def dem_cb(i):
        return data["demands"][mgr.IndexToNode(i)]
    demc= model.RegisterUnaryTransitCallback(dem_cb)
    model.AddDimensionWithVehicleCapacity(demc, 0, data["vehicle_capacities"], True, "Capacity")
    ...
    if sol:
        for vid in range(data["num_vehicles"]):
            idx, route, load, dist = model.Start(vid), [], 0, 0
